var AdminDataAdvanceSearch = (function($) {
	return {
		...
		init: function(table_structure) {
			...
			var $this = $('#advance_search_table');
			$this.append(first_row).filter('tr:first td:last a').hide() //hide the first delete row link
			.end().find('tr:first td:last').append('<a />').find('a').attr('href', '#').text('+').css({
				fontSize: '30px',
				textDecoration: 'none'
			}).click(function() {
				$this.append(AdminDataAdvanceSearch.buildRow(AdminDataJSUtil.random_number()));
				return false;
			});
		},
		...
		buildCol3: function(random_num) {
			return $('<td></td>').append('<input></input>').find(':input').attr({
		...
		buildCol2: function(random_num) {
			var that = this;
			return $('<td></td>').append('<select></select>').find('select').append('<option></option').addClass('ram_drop_down ram_drop_down_col2 disabled').attr({
		...
		buildCol4: function(random_num) {
			...
			return $('<td></td>').append('<a />').find('a').text('X').attr({
				id: random_num + '_remove_row',
				href: '#'
			}).addClass('remove_row').click(function() {
		...
		buildCol1: function(random_num) {
			var that = this;
			var select_col1 = $('<select></select>').addClass('ram_drop_down ram_drop_down_col1').attr({
				name: 'adv_search[' + random_num + '_row][col1]',
				id: random_num + '_adv_drop_down_col1'
			}).append($('<option />')); // first drop down option should be empty 
			for (var i in this.ts) {
				$('<option></option>').text(i).attr('value', i).appendTo(select_col1);
			}
			...
			return $('<td />').append(select_col1);
		},

		buildRow: function(random_num) {

			return $('<tr></tr>').append(this.buildCol1(random_num)).append(this.buildCol2(random_num)).append(this.buildCol3(random_num)).append(this.buildCol4(random_num)).attr('id', random_num);
		},
