...
AdminData.advanceSearch = {
	...
	buildFirstRow: function() {
		$('#advance_search_table').append(this.buildRow()).find('tr td:last a').text('+').removeClass('remove_row').addClass('add_row_link');
	},
	...
	buildCol2: function() {
		return $('<td />').append('<select />').find('select').append('<option />').addClass('col2 disabled').attr({
			disabled: true
		}).
		end();
	},
