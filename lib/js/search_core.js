jQuery(document).ready(function(){
 ...
 function build_sortby_row(){
  var tr = $('<tr></tr>');
  var td = $('<td></td>');

	var select_col = $('<select></select>').attr('name','sortby');

	for (var i in table_structure) {
			select_col.append($('<option></option>').html('&nbsp; ' + i + ' desc').attr('value',i+ ' desc'));
			select_col.append($('<option></option>').html('&nbsp; ' + i + ' asc').attr('value',i+ ' asc'));
		}

  var sortby = $('<span>sort by:&nbsp;</span>');
  return tr.append(td.append(sortby,select_col));
 }

	var first_row = build_row(random_number(1000000));
	$('#advance_search_table').append(first_row)
                            .append(build_add_another_row())
                            .append(build_sortby_row());
	...
	var options = {
	  success: function(responseText) {
		  ...
		  beforeSubmit: function() {
			$('#results').html('<span style=\'color:red;font-size:25px;margin:20px 0 20px 0;\'>searching ...</span>');
	  }
	};
