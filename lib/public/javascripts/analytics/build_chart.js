...
$(function(){
	...
	$table.find('a.build_chart_analytics_vs:first').click(function(){
		...
		f.find('td.operator select').attr('name', 'search[row_'+randomNumber+']operator');
		f.find('td.operator_value input').attr('name', 'search[row_'+randomNumber+']operator_value');
...
$('table.build_chart_analytics td.relationship select').live('change', function(e){
	...
	var with_type_value = $target.closest('tr').find('td.with_type select').val();
	
	if (value.replace(/\s/g) === '') {
		AdminData.analytics.disableOperator(e);
	} else {
		if (with_type_value === 'without') {
			AdminData.analytics.disableOperator(e);
		} else {
