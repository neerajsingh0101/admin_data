module AdminData
  class Util
    ...
    def self.google_hosted_jquery_ui_css(context)
      if Config.setting[:use_google_hosting_for_jquery]
    ...
    def self.is_allowed_to_view_feed?(controller)
      ...
      if Config.setting[:feed_authentication_user_id].blank?
        Rails.logger.info 'No user id has been supplied for feed'
        return false
      end

      if Config.setting[:feed_authentication_password].blank?
        Rails.logger.info 'No password has been supplied for feed'
        return false
      end

      stored_userid = Config.setting[:feed_authentication_user_id]
      stored_password = Config.setting[:feed_authentication_password]
    ...
    def self.custom_value_for_column(column, model)
      ...
      begin
        Config.setting[:column_settings].fetch(model.class.name.to_s).fetch(column.name.intern).call(model)
      rescue
    ...
    def self.columns_order(klasss)
      ...
      columns_order = Config.setting[:columns_order]
