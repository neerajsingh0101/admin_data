...
## How do I display a column in a specific way

I have an <tt>Article</tt> model which has <tt>title</tt> column. I alos have <tt>to_param</tt>
method defined like this.

<pre>
class Article < ActiveRecord::Base
 def to_param
    new_record? ? title.sluggify : "#{self.id.to_s}-#{title.sluggify}"
  end
end
</pre>

Whenever title is shown for the record, I would like to see the result of <tt>to_param</tt> method. Here 
is a way to achieve that.

<pre>
AdminDataConfig.set = ({
  :column_settings => {Article => {:title => lambda {|model| model.to_param}}} 
})
</pre>

Above pluging is being asked that while retrieving the value of 'title' column of 'Article' model invoke
the provided proc.

## How do I implement custom security 
...
Given below are two examples of custom security implementation.

### Custom security example 1

Requirement: 
I want to grant read access when method <tt>logged_in?</tt> returns true.
I want to grant update access when method <tt>admin_logged_in?</tt> returns true.
...
Note that for the above security model to work <tt>AppicationController</tt> should implemet
two methods <tt>logged_in?</tt> and <tt>admin_logged_in?</tt> .

This is just one example. Since user has access to <tt>controller</tt> in the proc user can call
any methd on the controller.

### Custom security example 2

Requirement:
I want to grant read and update access only to models <tt>User</tt> and <tt>Address</tt>. User should not be
able to update any other model. Also for the <tt>User</tt> and <tt>Address</tt> records, only the
records marked as <tt>unlocked</tt> should be allowed.

Implementation:

<pre>
 klasses = [User, Address]

AdminDataConfig.set = {
  :view_security_check => lambda {|controller| klasses.include? controller.model.class },
  :update_security_check => lambda {|controller| klasses.include?(controller.model.class) && 
